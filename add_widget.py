...
from PySide6.QtWidgets import QApplication,QMainWindow,QCompleter,QWidget,QButtonGroup
...
class AddWidget(QWidget,ui_realestate_form.Ui_Form):
    def __init__(self,on_ok):
        ...
        self.group.addButton(self.radioButton,1)
        self.group.addButton(self.radioButton_2,2)
    ...
    def row_edit(self,name,id,address,cost,check,description,type):
        self.name_edit.setText(name)
        self.address_edit.setText(address)
        self.value_box.setValue(cost)
        self.actuality_box.setChecked(check)
        self.description_edit.setText(description)
        self.group.button(type).setChecked(True)
        self.id = id
        self.show()


    def new_row_add(self):
        self.on_ok(self.name_edit.text(),self.id,self.address_edit.text(),self.value_box.value(),self.actuality_box.isChecked(),self.description_edit.toPlainText(),self.group.checkedId())
        self.id=None
        self.close()